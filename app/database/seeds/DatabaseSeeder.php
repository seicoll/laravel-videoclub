...
class DatabaseSeeder extends Seeder
{
    ...
    private $arrayPeliculas = array(
		...
		array(
			...
			'year' => '2014', 
			'director' => '', 
			'poster' => 'https://static.justwatch.com/poster/458725/s592/the-walking-dead', 
			'rented' => true, 
    ...
    public function run()
    {
        ...
        //Taula Movies
        self::seedCatalog();
        $this->command->info('Taula catálogo inicialitzada amb dades!');
        
        //Taula Users
	    self::seedUsers();
	    $this->command->info('Taula usuarios inicialitzada amb dades!');
        
        //Taula Chapters
	    self::seedChapters();
	    $this->command->info('Taula chapters inicialitzada amb dades!');
    }
    
    
    private function seedCatalog()
    {
        //Esborrem dades de la taula movies
        DB::table('movies')->delete();
        
        //Inserim les pel·licules
    ...
    private function seedUsers()
    {
        ...
        $user = new User;
	    $user->name = 'sergi';
	    $user->email = 'seicoll@gmail.com';
	    $user->password = bcrypt('patata');
	    $user->save();
    ...
    {
        //Esborrem dades de la taula chapters
        DB::table('chapters')->delete();
        
        //Inserim les reviews
        foreach( $this->arrayChapters as $chapter ) {
            $p = new Chapter;
            $p->title = $chapter['title'];
            $p->season = $chapter['season'];
            $p->episode = $chapter['episode'];
            $p->movie_id = $chapter['movie_id'];
            $p->save();
        }
    }
